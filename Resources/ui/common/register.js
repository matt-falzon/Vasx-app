...
function registerWindow(theArray){
	...
	w.add(msisdnTextField);
	//MSISDN text field events

	//event for button to close window when clicked
	b.addEventListener('click', function()
	{
		//stuff here
			Ti.App.msisdn = msisdnTextField.getValue();
		...
		if(Ti.App.network == "" || Ti.App.msisdn == "")
			...
			if(Ti.App.network.length > 0 && Ti.App.msisdn.length > 0 && Ti.App.country.length > 0 && Ti.App.url.length > 0){
				...
				if(rows.isValidRow() == false){//if not, insert new row
					...
					db.execute('INSERT INTO user_language (msisdn, lang_code, lang_name) VALUES(?,?,?)', Ti.App.msisdn, theArray[0].language[0].code, theArray[0].language[0].name);
				...
				db.close();
				vasxRegisterUser(Ti.App.msisdn, Ti.App.language, 1);
			}
			//login web service
	...
	table.addEventListener('click', function(e){
		...
		Ti.App.url = e.rowData.url.toString();///////////////////////////////
...
msisdnTextField.addEventListener('focus', function(){
	this.setBottom("80%");
	table.hide();
	swipeLabel.setVisible(true);
});
...
msisdnTextField.addEventListener('blur', function(){
	this.setBottom("20%");
	table.show();
	swipeLabel.setVisible(false);
});
...
//function used once the user presses the log in button, takes in the values of the
//username and password text fields
function vasxRegisterUser(msisdn, lang, vn){
...
var xml = generateXML("http://service.adminportal.vasx.com/", 'registerUser', parameters);
...
client.setRequestHeader('SOAPAction', '"registerUser"');
client.send(xml);

client.onload = function() {
    var results = client.responseXML.documentElement.getElementsByTagName("registerUserResponse");
