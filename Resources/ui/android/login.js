		...
		var freesms = xml_doc.getElementsByTagName('AvailableSMS');
			if(code.item(0).text == "AP-Success"){
 			...
 			setTimeout(function() {
	        	...
	        	var reg = require('ui/iphone/main');
...
loginButton.addEventListener('click',function(e){
	...
	var db = Ti.Database.open('local');
	var d = db.execute('SELECT country, site_name, url FROM data where msisdn = ?', usernameTextField.getValue());
	if(d.isValidRow()){
	...
	}else if(usernameTextField.getValue() == 'test', passwordTextField.getValue() == 'pass'){
 			var ui = require('ui/common/ui');
 			setTimeout(function() {
            	indicator.closeIndicator();
            	Ti.App.mainWin.setOpacity(1);
	        	...
	        	var reg = require('ui/iphone/main');
	        	var regWin = reg.main('test', usernameTextField.getValue(), '5', 'test', 'user', 'en');	
	}else{
		newUser(usernameTextField.getValue());
		indicator.closeIndicator();
	}
});
...
keychainLabel.addEventListener('click',function(){//check and uncheck the check_box
	if(check_box.getBackgroundImage() == 'images/common/checkbox.png') 
    	check_box.setBackgroundImagee('images/common/checkbox-checked.png');
    else 
...
passwordTextField.addEventListener('click', function(){
